...
export default function FarmerProducts() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const productData = {
      ...
      status: form.status.toLowerCase(), description: form.description,
      quality: form.quality, imageUrl: form.imageUrl, isOrganic: form.isOrganic,
    };
    if (editingId) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-gradient-to-r from-green-600 to-emerald-500 p-4 sm:p-6 rounded-2xl text-white shadow">
        ...
        <button
          onClick={() => { setOpen(true); setEditingId(null); setForm({ name: "", category: "", quantity: "", unit: "KG", price: "", status: "Available", description: "", quality: "Standard", imageUrl: "", isOrganic: false }); }}
          ...
          <div className="bg-white rounded-xl w-full max-w-2xl p-4 sm:p-6 relative shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <input name="imageUrl" value={form.imageUrl} placeholder="https://example.com/image.jpg" onChange={handleChange} className="border rounded-lg px-4 py-2 w-full" />
                {form.imageUrl && (
                  <div className="mt-2"><img src={form.imageUrl} alt="Preview" className="w-20 h-20 object-cover rounded border" /></div>
                )}
              </div>
