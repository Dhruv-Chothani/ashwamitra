...
const B2BCart: React.FC<B2BCartProps> = ({ cart, products, onUpdateCart, onGoToOrders }) => {
  ...
  const cartItems = Array.from(cart.entries()).map(([productId, quantity]) => {
    ...
    return { _id: productId, name: product.name, farmerName: product.farmerName || "Farmer", pricePerUnit: product.pricePerUnit, quantity, unit: product.unit || "kg", minimumOrder: product.minimumOrder || 1, imageUrl: product.imageUrl };
