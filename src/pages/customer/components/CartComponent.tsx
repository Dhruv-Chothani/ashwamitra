...
import { useCreateOrder, useCustomerOrders, useDeductWallet } from "@/hooks/useApi";
...
const CartComponent: React.FC<CartComponentProps> = ({ cart, products, onUpdateCart, onGoToOrders }) => {
  ...
  const deductWallet = useDeductWallet();
    ...
    try {
      ...
      if (useWallet && walletAmount > 0) {
        await deductWallet.mutateAsync({
          amount: walletAmount,
          description: `Order payment - ${cartItems.map(i => i.name).join(", ")}`
        });
      }
      ...
      // 3. Collect remaining amount through Razorpay when selected.
      ...
      // 4. Emit subscription event if applicable
              ...
              {orderPlaced ? (
                ...
                <>
                  ...
                  <button onClick={handlePlaceOrder}
                    disabled={createOrder.isPending || deductWallet.isPending || isRazorpayBlocked}
                    ...
                    {createOrder.isPending || deductWallet.isPending
                      ? "Processing..."
                      : isRazorpayBlocked
